...
def queries(cursor, cnx):
    cursor.execute("DROP TABLE IF EXISTS usuário, depósito, esportes, aposta, cassino")
    ...
    nome_usuário    VARCHAR(255) UNIQUE,
...
def inserir_queries(cursor, cnx):
    ...
    for i in range(1, 71):
        ...
        email_user = nome_usuario + random.choice(emails)
    ...
    #QUERY DEPÓSITO
    inserir_query = """
    INSERT INTO depósito (id_pagamento, id_usuário, nome_banco, pix, número_cartão, CVV_cartão, quantidade)
    VALUES (%s, %s, %s, %s, %s, %s, %s)
    """
    for i in range(1, 101):
        id_usuario = random.randint(1,70)
        id_pagamento = i
        ...
        _id = (id_pagamento, id_usuario, nome_banco, pix, n_cartao, cvv, quantidade)
        cursor.execute(inserir_query, _id)
        cnx.commit()
    ...
    for i in range(1,71):
        ...
        _id = (id_esporte, nome_jogo, jogo_dia, jogo_mes, jogo_ano, equipe1, equipe2)
        cursor.execute(inserir_query, _id)
        cnx.commit()

    #QUERY APOSTA
    inserir_query = """
    INSERT INTO Aposta (id_aposta, id_usuário, id_esporte, odds_equipe1, odds_equipe2, odds_escolhida, valor_aposta, valor_retorno)
    ...
    for i in range(1, 71):
        id_usuario = random.randint(1,70)
        id_esporte = i
        odds_equipe1 = round(random.uniform(1,7), 2)
        odds_equipe2 = round(random.uniform(1,7), 2)
        ...
        _id = (i, id_usuario, id_esporte, odds_equipe1, odds_equipe2, odds_escolhida, valor_aposta, valor_retorno)
        cursor.execute(inserir_query, _id)
        cnx.commit()
